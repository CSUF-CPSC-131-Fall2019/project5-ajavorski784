...
int main() {
  ...
  assertEquals("numberOfWords: Frankenstein", 9787,
               Frankenstein.numberOfWords());
  assertEquals("wordCount: Frankenstein", 16,
               Frankenstein.wordCount("Frankenstein"));
  ...
  assertEquals("mostFrequentWord: Frankenstein", "the",
               Frankenstein.mostFrequentWord());
  ...
  assertEquals("numberOfWords: Sleep Hollow", 3811,
               SleepyHollow.numberOfWords());
...
bool assertTrue(const string &nameOfTest, bool actual) {
  if (actual == true) {
    ...
    cout << GREEN << "PASSED " << RESET << nameOfTest
         << ": test successfully resulted in 'true'" << endl;
    testCount++;
    return true;
  }
  // Red colored text
  cout << RED << "FAILED " << RESET << nameOfTest
       << ": test unsuccessfully resulted in 'false'" << endl;
  return false;
}
...
bool assertEquals(const string &nameOfTest, double expected, double actual) {
  if (std::abs(expected - actual) < 0.005) {
    // Green colored text
    cout << GREEN << "PASSED " << RESET << nameOfTest
         << ": expected and actual '" << actual << "'" << endl;
    testCount++;
    return true;
  }
  // Red colored text
  cout << RED << "FAILED " << RESET << nameOfTest << ": expected '" << expected
...
bool assertEquals(const string &nameOfTest, const T &expected, const U &actual,
  ...
  if ((actual >= minValue) && (actual <= maxValue)) {
    // Green colored text
    cout << GREEN << "PASSED " << RESET << nameOfTest << ": expected '"
         << expected << "' close to actual '" << actual << "'" << endl;
    testCount++;
    return true;
  }
  // Red colored text
